...
import serviceAccount from '../config/asperinfotech-2d299-firebase-adminsdk-fbsvc-791770b0e0.json';
...
export const sendNotification = async (title: string, body: string, targetToken?: string, type: string = 'general') => {
    try {
        ...
        const messagePayload: any = {
            ...
            // Add notification block for automatic system display
            notification: {
                title: title,
                body: body,
            }
        ...
        if (response.ok) {
            console.log("✅ FCM V1 Sent Success:", json.name);
        } else {
            console.error("❌ FCM V1 Send Error Details:", JSON.stringify(json, null, 2));
        }
